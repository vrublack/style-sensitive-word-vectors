...
// The modified points from original `word2vec/word2vec.c` are as follows:
//  - Training style-sensitive word vectors separately from the syntactic/semantic-sensitive word vectors.
...
void *TrainModelThread(void *id) {
  ...
  real *neu1s = (real *)calloc(layer1s_size, sizeof(real));
  real *neu1es = (real *)calloc(layer1s_size, sizeof(real));
  ...
  while (1) {
    if (word_count - last_word_count > 10000) {
      ...
      if (alpha < starting_alpha * 0.0001) alpha = starting_alpha * 0.0001;     //minimun alpha
    }
    ...
    if (feof(fi) || (word_count > train_words / num_threads)) {
      ...
      last_word_count = 0;
      sentence_length = 0;
      // add variables for our training
      loss_iter = 0;
      loss_iter_s = 0;
      loss_cnt = 0;
      loss_cnt_s = 0;
    ...
    for (c = 0; c < layer1s_size; c++) neu1s[c] = 0;
    for (c = 0; c < layer1s_size; c++) neu1es[c] = 0;
    next_random = next_random * (unsigned long long)25214903917 + 11;

    // fixing or not the threshold between nearby (C^{near}) and far (C^{dist}) context words
    if (fixthr) threshold = iwindow;
    else threshold = next_random % iwindow + 1;
    ...
    if (cbow) {
      ...
      cw_ds = 0;
      cw_s = 0;
      for (context_position = 0; context_position <= sentence_length; context_position++) if (context_position != sentence_position) {
        if (context_position < 0) continue;
        if (context_position >= sentence_length) continue;
        last_word = sen[context_position];
      ...
      if (cw_ds || cw_s) {
        ...
        if (negative > 0) for (d = 0; d < negative + 1; d++) {
          ...
          if (use_near && cw_ds){
              ...
              loss_iter += g;
              loss_cnt++;
              for (c = 0; c < layer1_size; c++) neu1e[c] += (g * alpha) * syn1neg[c + l2];
              for (c = 0; c < layer1_size; c++) syn1neg[c + l2] += (g * (alpha / 3)) * neu1[c];   // consider learning rate and the expectation of the number of updates
          }
          ...
          if (use_dist && cw_s){
              ...
              loss_iter_s += g;
              loss_cnt_s++;
              for (c = 0; c < layer1s_size; c++) neu1es[c] += (g * alpha) * syn1neg[c + l2];
              for (c = 0; c < layer1s_size; c++) syn1neg[c + l2] += (g * (alpha / 3)) * neu1s[c];   // consider learning rate and the expectation of the number of updates
          }
        }
...
int main(int argc, char **argv) {
  ...
  if (argc == 1) {
    ...
    printf("\t-size <int>\n");
    printf("\t\tSet size of word vectors; default is 600\n");   // extend (original: default 100)
    printf("\t-sample <float>\n");
    printf("\t\tSet threshold for occurrence of words. Those that appear with higher frequency in the training data\n");
    printf("\t\twill be randomly down-sampled; default is 1e-3, useful range is (0, 1e-5)\n");
    // printf("\t-hs <int>\n"); // Unimplemented Hierarchical Softmax
    ...
    printf("\t-binary <int>\n");
    printf("\t\tSave the resulting vectors in binary moded; default is 1 (on)\n");  // binary on (original: default 0 (off))
    ...
    printf("\t-iwindow-threshold <int> \n");
    printf("\t\tinner window and threshold, ignoring closer context words to target word; default is 5\n");
    printf("\t-fix-threshold <int> \n");
    printf("\t\tif 1 then fix threshold size, else sample; default is 1\n");
    /* usage */
    printf("\nExamples:\n");
    printf("./stylisticword2vec -train data.txt -output vec.bin\n\n");
    return 0;
  }
  output_file[0] = 0;
  save_vocab_file[0] = 0;
